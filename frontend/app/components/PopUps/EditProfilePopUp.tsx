...
export default function EditProfilePopup({ onClose, onSave, user }: EditProfilePopupProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSave({ firstName, lastName, bio, profilePic: selectedImage || image }); // Pass the updated image URL or fallback to the original
  };
    ...
    <Modal isOpen={true} onClose={onClose} title="Profile Details">
      <form onSubmit={handleSubmit} className="flex gap-4">
        ...
        <div className="flex flex-col items-center">
          <img
            src={selectedImage || image || '/default-profile.png'} // Display the selected or original image
            alt="Profile"
