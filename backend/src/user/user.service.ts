...
export class UserService {
  ...
  async getFavoritePodcasts(id: string) {
    return this.UserModel.findById(id)
      ...
      .populate({
        ...
        select:
          'podcastName podcastDescription podcastImage categories creator',
        populate: {
          path: 'creator', // Get the creator of the podcast
          model: 'User',
  ...
  async getLikedEpisodes(id: string) {
    return this.UserModel.findById(id)
      ...
      .populate({
        ...
        populate: {
          // Nested populate to include podcast data
          ...
          select: 'podcastImage podcastTitle creator',
          populate: {
            path: 'creator', // Get the creator of the podcast
            model: 'User',
            select: 'firstName lastName ',
  ...
  async rateEpisode(userId: string, episodeId: string, value: number) {
    ...
    const averageRating =
      episode.ratings.reduce((acc, cur) => acc + cur.value, 0) /
      episode.ratings.length;
  ...
  // Avoid duplicates
  if (!user.following.includes(targetUserObjectId._id)) {
    user.following.push(targetUserObjectId._id);
    targetUser.followers.push(userObjectId._id);
  }
...
async unfollowUser(userId: string, targetUserId: string): Promise<any> {
  ...
  user.following = user.following.filter(
    (id) => id.toString() !== targetUserObjectId._id.toString(),
  );
  targetUser.followers = targetUser.followers.filter(
    (id) => id.toString() !== userObjectId._id.toString(),
  );
